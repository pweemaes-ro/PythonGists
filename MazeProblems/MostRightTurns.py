...
def count_right_turns(path: Path) -> int:
	"""Return the nr of right turns in the given path."""
	
	return sum(any([prev == (x + 1, y) and dest == (x, y - 1),
	                prev == (x, y + 1) and dest == (x + 1, y),
	                prev == (x - 1, y) and dest == (x, y + 1),
	                prev == (x, y - 1) and dest == (x - 1, y)])
	           for prev, (x, y), dest in zip(path, path[1:], path[2:]))
...
def find_all_paths(maze: Maze,
	...
	if (x, y) == finish:
		if is_valid_path(path):
			__all_path_infos.append((count_right_turns(path), path.copy()))
	else:
		for destination in [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)]:
			# todo: find more efficient way to check if destination in path...
...
def print_path(maze: Maze, path: Path) -> None:
	"""Print the path"""
